...
| **RULE-AI-003** | **No Silent Failures:** If the AI cannot extract a mandatory field (e.g., Amount) from an utterance, it MUST initiate a Clarification Dialogue (BP-004) rather than guessing or failing silently. | Fallback to Clarification flow if required fields are missing after NLP extraction. |
